...
const toneIcon = {
  ...
  info: <Info className="size-4 text-brand-600" aria-hidden />,
    ...
    <ToastPrimitive.Provider swipeDirection="right" duration={5000}>
      {toasts.map((t) => (
        <ToastPrimitive.Root
          ...
          className={cn(
            "flex items-start gap-3 rounded-overlay border border-line bg-surface p-4 shadow-lg",
          )}
        >
          {toneIcon[t.tone]}
          <div className="min-w-0 flex-1">
            <ToastPrimitive.Title className="text-sm font-medium text-ink-strong">
              {t.title}
            </ToastPrimitive.Title>
            {t.description ? (
              <ToastPrimitive.Description className="mt-0.5 text-sm text-ink">
                {t.description}
              </ToastPrimitive.Description>
            ) : null}
          </div>
          <ToastPrimitive.Close aria-label="关闭通知" className="rounded p-0.5 text-ink-muted hover:text-ink-strong">
