  ...
  return (
    <WorkbenchPageFrame width="workspace">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px]">
        <section className="flex min-h-0 items-center justify-center rounded-[var(--sh-radius-md)] bg-[var(--sh-surface-stage)] p-3 md:p-4 lg:max-h-[calc(100dvh-310px)]">
          <div
            className="w-full max-w-[min(960px,max(280px,calc((100dvh-342px)*1.7778)))] shadow-[var(--sh-shadow-floating)]"
          ...
          <div className="mt-3 flex w-full max-w-full gap-3 overflow-x-auto pb-1 lg:block lg:space-y-3 lg:overflow-visible lg:pb-0">
            {availableCandidates.map((candidate) => (
              <button
                ...
                className={`w-40 shrink-0 rounded-[var(--sh-radius-sm)] border bg-[var(--sh-surface-elevated)] p-2 text-left lg:w-full ${selectedId === candidate.id ? "border-[var(--sh-brand-500)] shadow-[var(--sh-shadow-card)]" : "border-[var(--sh-line-subtle)]"}`}
                disabled={approved}
                key={candidate.id}
                onClick={() => {
              ...
              >
                ...
                <span className="mt-2 block px-1 text-xs font-semibold text-[var(--sh-ink-strong)]">
                  {candidate.label}
                </span>
              </button>
            ))}
