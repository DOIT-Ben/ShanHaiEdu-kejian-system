...
| `/app`                                                     | `RuntimeAppShell` + `HomePage` | 品牌首页、真实项目摘要与独立创作入口                                |
...
| `/app/creation`                                            | `CreationHomePage`             | 独立图片、视频、PPT 创作入口                                        |
| `/app/creation/:studioPath`                                | `CreationStudioPage`           | 创建 standalone 批次；空 `items` 时阻断后续写链，不伪造结果         |
...
| `studioRegistry` | `features/creation-studio/registry.ts`                      | 图片、视频和 PPT 共用的生产创作台配置             |
...
| 创作批次       | `POST /creation-batches`；`POST /creation-batches/{batch_id}/generate`                                                                                                                         | `features/creation-studio/api/creationApi.ts`       | standalone 创建已接入；当前正式响应为 `items: []`，页面在此停止             |
| 创作四动作     | `POST /creation-items/{item_id}/prompt-versions`；`POST /creation-items/{item_id}/generate`；`POST /generation-results/{result_id}/adoptions`；`POST /adoptions/{adoption_id}/save-to-project` | `features/creation-studio/api/creationApi.ts`       | 客户端保留；没有服务端 CreationItem 时页面不得调用，采用/保存也保持阻断     |
| Generation Job | `GET /generation-jobs/{job_id}`；`POST /generation-jobs/{job_id}/cancel`                                                                                                                       | `features/jobs/api/jobsApi.ts`                      | 教材 setup 与已知 Job 深链已接入；standalone 创作当前不启动 Job             |
| SSE            | `GET /projects/{project_id}/events/stream`；`GET /generation-jobs/{job_id}/events/stream`                                                                                                      | `shared/api/useProjectEvents.ts`、`useJobEvents.ts` | Runtime 项目资源页和已知 Job/setup 用于 REST 对账；创作空 `items` 时不订阅  |
...
| 独立创作生成            | `POST /creation-batches` 可创建 standalone 批次；CreationItem 后续写客户端已存在                                                                                                                                                                                      | standalone 响应当前为 `items: []`，且 active OpenAPI 没有 CreationItem 创建端点（路径待 #11 决策）；页面不调用提示词、生成、Job REST/SSE。另缺批次、item 和 result GET，刷新恢复、预览、采用和保存均阻断         | #11       |
...
- 课时/教案生成、CreationItem 创建与创作结果查询恢复、正式 PPTX、最终视频和交付合同仍是 #11/#108 的页面阻塞；
