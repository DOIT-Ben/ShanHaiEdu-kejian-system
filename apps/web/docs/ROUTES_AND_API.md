...
| `/app/creation/:studioPath`                                | `CreationStudioPage`           | 创建 standalone 批次、保存提示词、启动生成并读取 Job；不伪造结果    |
...
| 创作批次       | `POST /creation-batches`；`POST /creation-batches/{batch_id}/generate`                                                                                                                         | `features/creation-studio/api/creationApi.ts`       | standalone 创建已接入；当前页面按 item 生成，未增加未使用的整批 wrapper      |
| 创作四动作     | `POST /creation-items/{item_id}/prompt-versions`；`POST /creation-items/{item_id}/generate`；`POST /generation-results/{result_id}/adoptions`；`POST /adoptions/{adoption_id}/save-to-project` | `features/creation-studio/api/creationApi.ts`       | 提示词保存与 item 生成已接入；缺 result GET 时无法取得 ID，采用/保存保持阻断 |
| Generation Job | `GET /generation-jobs/{job_id}`；`POST /generation-jobs/{job_id}/cancel`                                                                                                                       | `features/jobs/api/jobsApi.ts`                      | 教材 setup、独立 Job 深链与创作台已接入                                      |
| SSE            | `GET /projects/{project_id}/events/stream`；`GET /generation-jobs/{job_id}/events/stream`                                                                                                      | `shared/api/useProjectEvents.ts`、`useJobEvents.ts` | Runtime 项目资源页、Job/setup 与创作台用于 REST 对账                         |
...
| 创作结果恢复            | `CreationStudioPage` 已调用 `POST /creation-batches`、`POST /creation-items/{item_id}/prompt-versions`、`POST /creation-items/{item_id}/generate`，并通过 Job REST/SSE 对账；采用与保存写端点已有客户端                                                               | 没有 `GET /creation-batches/{batch_id}`、`GET /creation-items/{item_id}` 或 `GET /generation-results/{result_id}`；刷新后无法恢复，完成后无法读取候选 result ID，因此预览、采用和保存到项目均阻断                | #11       |
