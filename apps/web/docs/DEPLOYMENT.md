...
生产前端基线已由 PR #111 进入 `main`；#211 在其上增加真实 Session/CSRF 启动闭环，后续 R1 与约 6 秒视频黄金切片已经补齐教师可见链路。Issue #244 使用 `infra/prod` 建设公网 IP HTTPS、反向代理、独立生产 Compose、备份和回退；正式可用性仍以该 Issue/PR 的 exact SHA 公网验收记录为准。
