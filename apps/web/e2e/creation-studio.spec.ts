...
test("1440×900 使用上方展示区和底部悬浮输入台", async ({ page }) => {
  ...
  expect(previewBox?.width ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(820);
  expect(visualBox?.width ?? 0).toBeGreaterThanOrEqual(320);
  expect(visualBox?.width ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(440);
  expect(visualBox?.height ?? 0).toBeGreaterThanOrEqual(240);
  expect(visualBox?.height ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(330);
...
test("1280×800 首屏可查看结果并直接在底部调整", async ({ page }) => {
  ...
  expect(visual?.width ?? 0).toBeGreaterThanOrEqual(320);
  expect(visual?.width ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(440);
...
test("等效 110% 缩放时视频创作台保持紧凑", async ({ page }) => {
  ...
  expect(visual?.width ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(520);
...
test("项目图片与视频创作台继承来源上下文并共用资产队列", async ({ page }) => {
  await loginAsTeacher(page);
  const projectId = "01960000-0000-7000-8000-000000000001";
  const lessonId = "01960000-0000-7000-8000-000000000101";
  await page.goto(
    `/app/creation/images?projectId=${projectId}&lessonId=${lessonId}&package=video-assets&itemId=character`,
  );

  await expect(page.getByRole("link", { name: /制作镜头图片 当前/ })).toBeVisible();
  await expect(page.getByRole("textbox", { name: "画面内容" })).toHaveValue(/百分数的意义与读写/);
  await expect(page.getByRole("combobox", { name: "图片比例" })).toContainText("16:9");
  ...
  await page.getByRole("button", { name: "开始创作图片" }).click();
  await page.getByRole("button", { name: "打开项目资产" }).click();
  await expect(page.getByRole("button", { name: /取消场景/ })).toBeVisible();
  ...
  await page.getByRole("button", { name: "打开项目资产" }).click();
  await expect(page.getByRole("dialog", { name: "项目资产" })).toContainText("分镜");
});
