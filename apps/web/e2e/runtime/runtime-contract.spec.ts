...
test("真实模式首页、项目概览和生产 Session 启动只消费合同接口", async ({ page }) => {
  ...
  expect(api.sessionReads).toBeGreaterThan(0);
...
test("真实模式在刷新后沿用创建意图并完成上传、任务和概览恢复", async ({ page }) => {
  ...
  expect(api.csrfHeaders.every((value) => value === runtimeSessionCsrfToken)).toBe(true);
