...
test("真实模式首页、项目概览和安全会话门禁只消费合同接口", async ({ page }) => {
  ...
  await expect(page.getByText("暂时无法保存，请刷新页面后重试。")).toBeVisible();
  ...
  await expect(page.getByText("暂时不能取消任务，请刷新页面后重试。")).toBeVisible();
...
test("真实模式在刷新后沿用创建意图并完成上传、任务和概览恢复", async ({ page }) => {
  ...
  await page.goto("/app/projects/new");
  await expect(page.getByText("暂时无法保存，请刷新页面后重试。")).toHaveCount(0);
