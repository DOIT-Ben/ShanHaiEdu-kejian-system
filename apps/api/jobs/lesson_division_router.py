"""Exact lesson-division GenerationJob recovery query."""
...
@router.get(
    "/api/v2/projects/{project_id}/lesson-division/generation-jobs",
    response_model=GenerationJobListEnvelope,
    operation_id="listLessonDivisionGenerationJobs",
)
def list_lesson_division_generation_jobs(
    project_id: UUID,
    request: Request,
    ...
    jobs = GenerationJobRepository(session, actor.organization_id).list_lesson_division_jobs(
        project_id
    )
