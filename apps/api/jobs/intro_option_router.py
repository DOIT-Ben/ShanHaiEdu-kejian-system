"""Exact Intro option GenerationJob recovery query."""
...
@router.get(
    "/api/v2/projects/{project_id}/lessons/{lesson_id}/intro-options/generation-jobs",
    response_model=GenerationJobListEnvelope,
    operation_id="listIntroOptionGenerationJobs",
)
def list_intro_option_generation_jobs(
    project_id: UUID,
    lesson_id: UUID,
    request: Request,
    ...
    jobs = GenerationJobRepository(session, actor.organization_id).list_intro_option_jobs(
        project_id,
        lesson_id,
    )
