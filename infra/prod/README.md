...
2. 从 `env.example` 创建 `shared/production.env`，写入公网 IP、exact SHA 和固定 Principal ID，权限设为 `0600`。
   ...
   `SHANHAI_IMAGE_SOURCE` 默认为 `build`。共享主机无法安全承担镜像构建时，可显式设为 `preloaded`；此模式只接受已加载且 `org.opencontainers.image.revision` 标签与本次 exact release SHA 完全一致的 API/Web 镜像，缺失或不匹配会在任何数据操作前停止。
3. 执行：
