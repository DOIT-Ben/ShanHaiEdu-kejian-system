...
`release.sh` 与 `rollback.sh` 通过 `shared/operations.lock` 持有独占锁；`monitor.sh` 使用同一文件的共享非阻塞锁。systemd healthcheck 也会在解析 `current` 下的监控脚本前获取共享非阻塞锁，使回退到不含此协调逻辑的旧应用版本时仍保持发布互斥。已经开始的监控会在换容器前自然完成，发布或回退进行中触发的定时监控会成功跳过，避免把受控容器替换误报为应用故障。
