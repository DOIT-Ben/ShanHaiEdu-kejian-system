...
MinIO 备份/恢复和生产 bootstrap 命令的完整 `docker compose` 执行边界都不得把原始输出写入发布日志，因为容器入口、Shell 基础命令或业务命令都可能包含对象路径或内部标识。`release.sh` 丢弃这些命令的 stdout/stderr，只保留固定的脱敏阶段错误，同时继续按原退出码失败关闭并触发既有应用回退。
