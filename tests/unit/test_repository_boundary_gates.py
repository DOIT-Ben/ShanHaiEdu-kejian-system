...
def test_live_size_baseline_is_exactly_two_files_and_twenty_three_functions(
    capsys,
) -> None:
    ...
    assert errors == []
    assert len(baseline.oversized_files) == 2
    assert len(baseline.oversized_functions) == 23
    report = capsys.readouterr().err
    assert report.count("warning: oversized file:") == 2
