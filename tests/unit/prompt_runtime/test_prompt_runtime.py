...
def test_teacher_prompt_keeps_full_context_content_without_source_metadata() -> None:
    compiled = compile_prompt(
        ...
        context=assemble_context(
            (binding("material", "material.approved_parse", exposure="full"),),
            {
                "material.approved_parse": (
                    item(
                        "material.approved_parse", "material-private-id", {"text": "Visible fact"}
                    ),
                )
            },
        ),
    ...
    assert "Visible fact" in compiled.preview.editable_prompt
    assert "material-private-id" not in compiled.preview.editable_prompt
    assert "source_version_id" not in compiled.preview.editable_prompt
