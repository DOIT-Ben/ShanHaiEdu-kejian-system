...
def seed_queued_job(session) -> GenerationJob:
...
def test_expired_worker_lease_is_recovered_and_duplicate_delivery_is_absorbed(
    ...
    with factory() as session, session.begin():
        job = seed_queued_job(session)
        job_id = job.id

    with factory() as session, session.begin():
        first = GenerationJobService(session, idempotency_ttl_seconds=900).claim(
            job_id, worker_id="worker-1", lease_seconds=5
        )
        assert first is not None and first.status == "running"

    with factory() as session, session.begin():
        duplicate_active = GenerationJobService(session, idempotency_ttl_seconds=900).claim(
            job_id, worker_id="worker-2", lease_seconds=5
        )
    ...
    with factory() as session, session.begin():
        recovered = GenerationJobService(session, idempotency_ttl_seconds=900).claim(
            job_id, worker_id="worker-2", lease_seconds=5
        )
    ...
    with factory() as session, session.begin():
        completed = GenerationJobService(session, idempotency_ttl_seconds=900).complete(
            job_id, worker_id="worker-2"
        )
        assert completed is not None and completed.status == "succeeded"

    with factory() as session, session.begin():
        duplicate = GenerationJobService(session, idempotency_ttl_seconds=900).complete(
            job_id, worker_id="worker-2"
        )
...
def test_cancel_requested_before_claim_finishes_as_cancelled(
    ...
    with factory() as session, session.begin():
        cancelled = GenerationJobService(session, idempotency_ttl_seconds=900).request_cancel(
            job_id,
    ...
    with factory() as session, session.begin():
        claimed = GenerationJobService(session, idempotency_ttl_seconds=900).claim(
            job_id, worker_id="worker-after-cancel", lease_seconds=5
        )
