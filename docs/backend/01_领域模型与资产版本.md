...
- `derives_from`：下游内容由上游生成或实质转换；
- `references`：生成或编辑时引用；
- `constrains`：上游规则或批准事实约束下游有效性；
- `supersedes`：新版本替代旧版本。
`renders` 是结构化Artifact到FileAssetVersion的派生文件关系，`selects` 是IntroSelection或Adoption的选择事实；二者不属于ArtifactVersion到ArtifactVersion的四值关系集合。
...
Artifact 的批准要求由项目固定的 ContentRelease 与 WorkflowDefinitionVersion 反向解析。发布绑定明确声明 `quality_requirement.mode=reports` 时，用户和系统身份都只能批准当前 submitted ArtifactVersion，且必须存在同项目、课时、内容哈希、Release、Workflow、validator set 和已通过 validate NodeRun 全部精确匹配的不可变 `ArtifactQualityReport`。批准证据中的 `report_id` 与 `evidence_hash` 只由服务端写入；客户端和内部调用方不能传入自由质量证据。`request_changes` 不依赖 passing 报告，新提交必须形成自己的报告，旧报告和旧 Approval 只保留历史事实。

## 7. 导入方案选择

`intro_option_set` 的默认版本包含九套完整方案。生成时保存一份只包含目标课时、知识点、学习目标、内容边界、不得提前讲授、年级/年龄、教材证据摘要和可选教师偏好的 `ContextSnapshot`；最终版本与每套方案都保留课时和知识点追溯。

`IntroSelection` 引用一个exact `approval:intro_option_set`、其ArtifactVersion和其中稳定 `option_key`，记录 `teacher_selected` 或 `policy_default`、选择者、策略版本、推荐度证据和时间。`policy_default`只在固定AutomationPolicy对`intro.select`显式允许`auto_select`时成立。修改选择新增记录并让旧选择失效，不回写方案集或教案。
