...
宿主机上的Git、`compose.sh`、uv和测试命令统一由`shanhai-dev`执行；root只承担已经明确授权的主机运维，不运行Git，不启动开发Compose，不执行项目测试。Issue worktree只在`/srv/shanhaiedu/worktrees/`下创建，主仓不为任务切换分支。容器只读挂载Git元数据，任何需要写ref、object、reflog或worktree登记的操作都回到宿主机并使用`shanhai-dev`。
...
每次接手、fetch失败或Git报告对象、ref、reflog、worktree权限异常时，先执行无修改检测。下面所有Git和文件可访问性检查都以`shanhai-dev`运行；任何输出或非零退出都视为漂移，不用root重跑Git来绕过错误：
...
sudo -u shanhai-dev -- find "$git_common_dir" -xdev \
  \( ! -user shanhai-dev -o ! -readable \) -print
sudo -u shanhai-dev -- find "$repo" /srv/shanhaiedu/worktrees -xdev -maxdepth 2 \
  -name .git \( ! -user shanhai-dev -o ! -readable \) -print
```
...
1. 由受控管理员把Git公共元数据及其UID、GID、mode、ACL和扩展属性清单备份到`/srv/shanhaiedu/backups/<timestamp>/`；备份不包含工作区密钥、运行数据或用户素材。对归档执行可读性列举并记录SHA-256，二者通过后才允许改属主。
2. 核对异常清单中的每个绝对路径都位于已解析的Git公共目录内，并确认没有符号链接越界。只对清单中的单个精确路径逐项执行`chown -- shanhai-dev:shanhai-dev <exact-path>`；仅在原mode本身异常且已记录时逐项修正mode。禁止`chown -R`、`chmod -R`、对仓库父目录批量改权或删除/重建`.git`。
...
4. 若只变更了属主或mode，回退时按备份清单对同一批精确路径逐项恢复原UID、GID和mode；若发现对象、ref、reflog或worktree内容损坏，停止权限操作并保留现场，由受控管理员从已验证归档制定单独恢复方案。

备份不可读、归档摘要不匹配、异常路径越出Git公共目录、需要改变Git内容、复验仍报对象或ref错误，或第二次精确修复后仍出现新漂移时，必须升级为独立基础设施事故。此时禁止继续等价重试、全仓改权、重新clone覆盖现场或删除对象；保留备份、权限清单和脱敏命令结果，并在Issue中记录下一步最小动作。
