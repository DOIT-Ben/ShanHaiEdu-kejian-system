...
宿主机上的Git、`fsck`、`fetch`、`status`、`worktree`、`compose.sh`、uv和测试命令统一由`shanhai-dev`执行；root只承担已明确授权的主机运维，不运行Git，不启动开发Compose，不执行项目测试。Issue worktree只在`/srv/shanhaiedu/worktrees/`下创建，主仓不为任务切换分支。容器只读挂载Git元数据，任何需要写ref、object、reflog或worktree登记的操作都回到宿主机并使用`shanhai-dev`。
