...
默认使用Squash Merge，PR标题作为主分支提交信息。鉴权、核心数据表、破坏性合同、模型费用策略和发布基础设施由对应CODEOWNER审核。
...
1. 验收标准通过。
2. 必需检查通过并记录证据。
3. 数据、合同、实现、测试和现行文档一致。
4. PR已Squash Merge。
5. Issue通过`Closes`自动关闭。
6. 远端任务分支已删除。
7. 本地分支和worktree已清理。
8. 任务导致进入新里程碑、可演示能力实质变化、新增或解除阶段阻塞，或下一阶段出口改变时，已在同一PR更新`CURRENT_STATUS.md`；不触发时已在PR明确声明。
