...
- 首套内置包当前正式发布为`1.0.0`；#130与PR #141已经把同一47节点目录（22个`model_generation`、13个`deterministic`、12个`human_gate`）的v2拓扑、模型输出投影和质量/审核声明作为`1.1.0`候选合入主线，#146继续修复其安全与发布固定性缺口。该候选只有#146合并并通过PostgreSQL前向发布复验后才成为新项目默认；黄金Fixture只作为确定性开发基线，阶段出口仍使用真实教材和真实文本模型。
