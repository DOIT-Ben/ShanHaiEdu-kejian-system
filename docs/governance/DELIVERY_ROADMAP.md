...
- 首套内置包当前正式发布为`1.0.0`；#130负责把同一47节点目录（22个`model_generation`、13个`deterministic`、12个`human_gate`）的v2拓扑、模型输出投影和质量/审核声明前向发布为`1.1.0`。该候选只有合并并通过PostgreSQL前向发布验证后才成为新项目默认；黄金Fixture只作为确定性开发基线，阶段出口仍使用真实教材和真实文本模型。
