...
- 当前生产路由以`apps/web/src/app/RuntimeApp.tsx`为准；登录使用`/login`，课时工作台使用`/app/projects/:projectId/lessons/:lessonId/work/:stepKey`，不得用计划路由或别名声明完成。
- 真实API Playwright固定放在`e2e/real-api/`并使用专用配置、命令和CI启动FastAPI、PostgreSQL与Redis；请求拦截和浏览器内伪造会话不属于真实联调。
- 改变生产交付边界的PR必须同步改变`contracts/delivery-slices/<issue>-<slice>.yaml`，在同一行绑定页面路由、真实导航、active API方法/路径、SQLAlchemy事实、后端集成测试和真实API Playwright；不能从几个互不相关的测试拼接完成证据。
...
- 专用真实API Playwright覆盖关键链路和刷新恢复；
- 清单声明的精确后端与浏览器选择器均实际执行且无skip、xfail或fixme；
