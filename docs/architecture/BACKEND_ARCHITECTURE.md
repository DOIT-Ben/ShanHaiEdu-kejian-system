...
> 当前状态：本文件是架构摘要。完整且优先的运行时设计见`docs/superpowers/specs/2026-07-17-shanhai-courseware-platform-runtime-design.md`。如本文与完整设计冲突，以完整设计为准。
...
apps/api/app/
  ...
  core/                   配置、认证、错误、日志、幂等
  modules/
    projects/             项目、课时、分支和成员
    textbooks/            教材上传、解析、引用
    lesson_plans/         课时划分与动态结构教案
    workflows/            DAG、节点状态、推进策略
    content_runtime/      内容包、动态结构、上下文和Prompt编译
    assets/               产物、文件、资产位、版本和派生关系
    creation/             创作包、批次、候选和保存回项目
    studios/              图片、视频、PPT通用创作入口
    ppt/                  大纲、逐页规格、视觉契约和装配
    video/                锚点、故事、分镜、shot、clip和合成
    generations/          Job、Attempt、Result和运行记录
...
| 对象 | 关键字段与职责 |
|---|---|
| project | 一个小知识点教材项目、学科、年级、教材版本和默认运行策略 |
| lesson_unit | 项目内稳定课时、顺序、标题和批准的课时规划版本 |
...
| project_asset_slot | 项目中稳定目标位置，例如PPT页图、shot关键帧或clip |
| artifact_version | 教案、PPT、脚本、分镜和视频的业务版本 |
| content_definition_version | 可导入的章节、嵌套字段、校验、UI和导出结构；十二部分只是默认版本 |
| creation_package | 项目导入创作台的一次不可变输入快照 |
| creation_batch | 创作台可编辑批次及批量任务状态 |
| generation_result | 模型候选结果，选择并保存后才能进入项目资产包 |
...
```text
disabled / not_ready / ready / draft / queued / running
review_required / approved / partially_completed / failed
paused / cancel_requested / cancelled / stale / skipped
```

节点状态、生成任务状态、候选审核状态和保存状态分开建模。内部Provider状态映射为任务状态，不得直接覆盖业务节点状态。
...
1. 上传教材并形成可追溯解析结果。
2. 划分课时并按已固定的内容结构版本生成每课时教案；内置默认结构为十二部分。
3. 在课程内容隔离条件下生成三类九套锚点候选并保存批准选择。
4. 生成和保存最终提示词、输出与版本。
5. 完成项目创作包导入通用创作台、候选筛选和原子保存回项目。
6. 完成封面门禁、正文纯白、逐页资产和可编辑混合PPTX。
7. 按母版故事、粗分镜、视觉母图、图片资产和细分镜生成可重试的视频片段。
8. 只有合格候选被选择并保存到shot资产位后才形成正式clip，再合成可播放视频。
9. 刷新、退出、失败重试后状态和资产不丢失。
10. 管理员能导入内容包、配置模型路由并查看脱敏运行记录。
11. 真实模型调用、费用、错误和产物可审计，不用占位内容冒充完成。
