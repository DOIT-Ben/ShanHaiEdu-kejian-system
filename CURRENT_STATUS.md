...
- `main`要求一次独立批准，但仓库当前唯一协作者与PR #66作者均为`DOIT-Ben`；合并前必须增加一名可审核的独立协作者，不能自批或使用管理员绕过。
...
结构化字段未确认前，不扩展#48后续实现，也不把#44的工程检查通过解释为具体业务字段已经批准。#29当前等待受控真实文本冒烟和独立审核人；两个门禁满足后继续PR #66的出口复验、评审、合并和main复验。
