...
- PR #66必须由未参与实现的只读子智能体审查最终base-to-head diff；审查证据绑定精确base/head SHA，任何HEAD变化后必须重新审查，P0/P1关闭且P2/P3修复或显式接受后方可转Ready和合并。该工程批准不要求另一GitHub账号提交`APPROVED`。
...
结构化字段未确认前，不扩展#48后续实现，也不把#44的工程检查通过解释为具体业务字段已经批准。#29当前需关闭子智能体审查发现并完成受控真实文本冒烟；两个门禁满足后继续PR #66的出口复验、最终子智能体审查、合并和main复验。
