...
验证基线：`main`；#130任务分支中的`1.1.0`/v2只代表待合并候选，实现事实以本文件所在提交的代码、迁移和测试为准；任务状态以链接的GitHub Issue和Pull Request为准
...
- 主线已发布的`shanhai.primary_math.courseware@1.0.0`已经为22个模型节点固定输入、Prompt、输出、教师可读投影和逻辑能力，但尚不包含#130新增的v2 `output_persistence`；三类九套由课程驱动单节点直接生成最终方案集。“1～5的认识”黄金Fixture可以分别启动教案、PPT和视频合同测试，但不代表真实模型或媒体生产完成。
...
- [Issue #130](https://github.com/DOIT-Ben/ShanHaiEdu-kejian-system/issues/130)：在#123合并后串行补齐模型输出的Artifact/CreationPackage持久化投影；当前为`status:in-progress`，分支中的`shanhai.primary_math.courseware@1.1.0`和Binding v2正在实现与验证，合并并通过PostgreSQL前向发布验证前不属于`main`或当前运行时默认。
...
- #48已有机器合同和黄金数据，但主线尚无读取数据库已发布GenerationTemplate并完成Prompt/Context冻结、模型调用、结构校验和Artifact原子落库的通用执行器；#51、#90、#112、#86与#123已经完成，#130尚在收口纯投影合同及PostgreSQL前向发布验证，#89仍等待#130合并后在最新`main`上对六项门禁完成新鲜复验。
...
进入阶段2“教材到教案纵向链路”时，#51、#88、#90、#112、#86和#123已经完成，当前按[交付路线](docs/governance/DELIVERY_ROADMAP.md)推进#130。任一时刻后端主任务不超过三个，修改同一合同或跨模块事务的任务不得并发。#89通用执行器仅在#51、#90、#112、#86、#123和#130全部合并并从最新`main`完成新鲜复验后单独启动。随后基于已发布黄金输入交付课时/教案与三类九套/选择快照；PPT/图片和视频在阶段2运行时稳定后使用独立短分支并行推进。TTS继续延后，#11等待生产前端达到联调条件。所有新任务使用仓库外隔离短worktree；同一Issue只保留一个任务分支和一个活动现场。
...
新对话继续接手#130唯一任务分支和对应活动worktree，不另建同名现场；#51、#90、#112、#86与#123已经完成，#89仍须等待#130合并并在最新`main`上对六项门禁完成新鲜复验。#88已完成`1.0.0`正式内容发布；#130的`1.1.0`/v2在合并和PostgreSQL前向发布验证前仍是候选，不得回退到Fixture常量、覆盖既有项目Release或称为当前运行时默认。当前API客户端只从runtime合同生成，planned合同不可用于联调。不得从外部旧Skill恢复七部分教案、固定50秒视频、视频读取教案/PPT、第二套DTO或把TemplateDraft编译器当成模型执行运行时。
