...
验证基线：`main`；#130与PR #141、#146与PR #148、#89与PR #147、#133与PR #164、#134与PR #174、#125与PR #177、#126与PR #178已经合并，#116 Intro合同校准已在当前树实现，Decision #153已经关闭；`1.1.0`/v2继续保持待显式发布候选且旧项目绑定不变；实现事实以本文件所在提交的代码、迁移和测试为准，任务状态以链接的GitHub Issue和Pull Request为准

当前阶段：阶段0出口尚未关闭；阶段1通用口径校准、后端轨道、首套业务机器合同和后端治理审计已经关闭，阶段2共享运行时已完成#146安全与发布固定性收口、#89通用节点执行闭环、#131字段级编辑权限、#133不可变质量报告、#134 exact批准守卫、#125课时划分、#126逐课时教案运行时和#116 Intro合同校准；下一项按#127方案集运行时推进
...
- 正式发布的`shanhai.primary_math.courseware@1.0.0`继续固定原内容、v1工作流和既有项目绑定；当前树的`1.1.0`/v2前向候选已由#116补齐48节点Intro exact来源、独立批准与选择合同，并可通过显式PostgreSQL发布原子成为后续新项目默认，但尚未宣称生产环境已执行该发布。“1～5的认识”黄金Fixture可以分别启动教案、PPT和视频合同测试，但不代表真实模型或媒体生产完成。
...
- [Issue #127](https://github.com/DOIT-Ben/ShanHaiEdu-kejian-system/issues/127)：方案集生成、校验与批准运行时等待#116合并复验；随后从最新`main`转ready，不在#116分支提前实现业务服务。
...
- #48的机器合同和黄金数据已由#89接入通用执行器，#131字段级编辑权限、#133不可变质量报告、#134批准守卫、#125课时划分、#126教案运行时与#116 Intro合同已完成或在当前树实现；#127在#116合并复验前保持blocked，不得绕过exact来源、独立批准或把确定性Fake冒充真实Provider或媒体出口。
...
通用口径校准、首套业务内容合同、#100课程驱动纠偏、#88黄金内容发布、阶段1后端轨道、#51 Prompt公共投影、#90 Attempt恢复、#86关系语义、#123 Prompt合同收窄、#130输出投影、#146安全收口、#89通用执行器、#131字段级编辑权限、#133不可变ArtifactQualityReport、#134 exact批准守卫、#125课时划分、#126逐课时教案运行时和#116 Intro合同校准已经完成或在当前树实现。#116合并复验后按#127方案集运行时、#128选择快照、#129 runtime API顺序推进。阶段0前端联调出口继续独立等待生产前端源码交接。
...
进入阶段2“教材到教案纵向链路”时，#51、#88、#90、#112、#86、#123、#130、#146、#89、#131、#133、#134、#125、#126和#116已经完成或在当前树实现。任一时刻后端主任务不超过三个，修改同一合同或跨模块事务的任务不得并发。下一步#127消费同一套NodeRun、Artifact、QualityReport与Approval内核实现一套/九套方案集，#128再建立不可变选择快照，#129最后提升runtime API。PPT/图片和视频继续走各自已批准的独立任务，不与Intro任务并发修改共享合同。TTS继续延后，#11等待生产前端达到联调条件。所有新任务使用仓库外隔离短worktree；同一Issue只保留一个任务分支和一个活动现场。
...
新对话先从最新`main`复验#116；确认Issue关闭、PR合并和分支清理后再把#127转ready并创建隔离实现分支。不恢复已撤销的云端、#130现场或已清理的#89/#125/#126工作树。#88正式发布的`1.0.0`及既有项目绑定必须保持不变；`1.1.0`/v2继续由显式发布命令前向激活，不能把测试数据库发布冒充生产已执行。当前API客户端只从runtime合同生成，planned合同不可用于联调。不得从外部旧Skill恢复七部分教案、固定50秒视频、视频读取教案/PPT、第二套DTO或把TemplateDraft编译器当成模型执行运行时。
