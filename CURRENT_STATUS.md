...
3. #205使用一个跨栈短分支和PR：新增`1.5.0`最小前向绑定，并在现有教师页面完成“上传或选择唯一关键帧 -> 生成6秒级真实短片 -> 轮询状态 -> 播放 -> 采用 -> 刷新恢复”；只复用真实视频Adapter、关键帧relay、NodeRun、GenerationResult/Adoption、FileAssetVersion与项目资产合同，不恢复完整视频链。
