...
当前阶段：阶段1后端基座已经支撑阶段4教师可见小学数学MVP完成，并以公网IP HTTPS进入受控内测；教材范围、课时划分、十二部分教案、三类九套、约6秒课堂导入短片、真实文本/视频黄金项目和生产发布均已形成可复验结果。项目下一步进入阶段5产品化，但尚未批准新的产品纵向切片。
> 最后核验：2026-08-03。
> 生产运行`main@8ec831072c643c7bc9b4cdcf2d240fd3f469bedd`，入口为`https://121.40.117.240`；首次开放范围仍是access code受控内测。普通开发与CI继续只使用确定性Fake。
...
- 生产发布具备exact SHA镜像来源、IP TLS、独立Compose/卷/Secret、PostgreSQL与MinIO备份恢复、五分钟健康监控及应用回退；公网真实Chrome登录、项目创建、刷新恢复和登出负测为`3/3`通过。

## 已完成

- [Issue #244](https://github.com/DOIT-Ben/ShanHaiEdu-kejian-system/issues/244)完成正式公网发布Decision：PR #254 至 #268依次交付生产拓扑、来源校验、入口、健康、发布互斥和exact环境切换；最终生产`current`、环境文件与运行容器均绑定`8ec8310...`，回退目标为`6b15cf7...`，新到旧再到新的应用回退演练通过。
- #244的Alembic、内容与身份初始化、PostgreSQL/MinIO pre/post备份与独立恢复、六容器健康、HTTP到HTTPS跳转、IP证书、monitor和外部真实API Playwright均已复验；详细脱敏证据保存在Issue，不在状态页复制任务日志。
...
- 当前没有已批准且处于`status:in-progress`的产品实现Issue。阶段5的下一条教师纵向切片必须先建立或更新独立Decision Issue，再进入实现。
...
- 当前没有已知Session/CSRF、PostgreSQL、Worker、active OpenAPI、生产页面、真实文本/视频黄金验收或视频对象生命周期阻塞。
- 生产与其他服务共享同一ECS的资源争用和共同故障域风险已在#244明确接受。一次高负载窗口中的Redis readiness超时已由下一轮timer自然恢复；生产容器无OOM或重启，后续继续由五分钟monitor暴露同类风险。
- 阶段5产品方向尚未决定。PPT/PPTX、完整图片链、完整视频链和TTS不得并行铺开，也不得直接恢复历史`status:blocked`实现；必须先选择一个教师可见结果并重新确认范围、合同、预算和浏览器验收。

## 下一个阶段出口

1. 创建并批准一个阶段5 Decision Issue，只选择PPT/PPTX、完整图片链、完整视频链或TTS中的一条最小教师纵向切片。
2. 将选定结果拆成一个唯一owner、一个短分支和一个Draft PR可独立验收的实现Issue；先写失败测试，再复用现有Session/CSRF、项目、节点运行、任务、资产、采用、对象存储、OpenAPI和生成客户端。
3. 普通CI保持确定性Fake；阶段出口必须在生产级页面、真实API/PostgreSQL、真实文件和明确预算下完成受控验收，再由未参与实现的独立reviewer绑定final exact base/head。

## 接手提示

1. 先读`README.md`、`AGENTS.md`、`docs/governance/项目记忆与接手索引.md`和本文件，再实时fetch并核验Issue、PR、分支、生产SHA与工作区。
2. R1、约6秒视频黄金切片和#244生产发布均已完成；不得恢复#222、#230或按技术层严格串行的#223至#229旧执行方式。
3. 生产部署继续遵循`infra/prod/README.md`，保留独立目录、Compose项目、卷、网络、Secret、备份、监控和可回退exact SHA；不得把共享ECS描述为物理隔离。
4. 新产品方向必须回到阶段5 Decision Issue；历史blocked Issue只能作为只读输入，不能自行授予实施范围。
