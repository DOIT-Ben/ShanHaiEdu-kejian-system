...
阿里云Linux workspace容器是生产级代码的主要开发环境，Windows只保留应急回退；正式生产部署由后续独立任务建设。宿主机只需要Git、Docker Engine和Docker Compose 2.40以上；Python、Node、uv、pnpm、FFmpeg、LibreOffice和项目依赖由版本化镜像提供。首次建立环境或环境合同变化时构建一次：
...
包装脚本按worktree目录派生Compose项目名，并把Git公共元数据只读挂载给测试，不让容器写Git仓库。本地`.env`、数据库、缓存和对象存储数据不得提交。普通测试和CI不连接真实模型Provider。完整拓扑、并行worktree、迁移和回退规则见[Linux开发环境与迁移](docs/governance/Linux开发环境与迁移.md)。
